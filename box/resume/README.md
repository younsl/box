...
My minimalistic resume built with native HTML, CSS, and JavaScript with language toggle functionality.

## Usage

Open resume in browser or generate PDF:

```bash
make open  # Open resume with language toggle in Chrome
make pdf   # Generate PDF from HTML file
make help  # Show all available commands
```

The resume includes a language toggle button (EN/KO) in the top-right corner to switch between English and Korean content.
...
/Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome \
  ...
  --print-to-pdf=$(OUTPUT_PDF) $(RESUME_HTML)
```
