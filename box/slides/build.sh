...
for slide_path in "${files_to_convert[@]}"; do
    ...
    if [ $DRY_RUN -eq 1 ]; then
        ...
        echo "find \"$dir_path\" -type f -name \"*.pdf\" -print -exec rm {} +"
        ...
        echo "Removing existing PDF files in $dir_path:"
        # 각 디렉토리에서 PDF 파일들을 찾아서 출력하고 삭제
        find "$dir_path" -type f -name "*.pdf" -print -exec rm {} +
        cd "$dir_path" && \
        podman run --rm --init \
            -v $PWD:/home/marp/app/ \
            -e LANG=$LANG \
