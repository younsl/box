# pnpm-lock.yaml is absent 오류 해결 가이드

## 개요

CI/CD 파이프라인에서 pnpm을 사용할 때 다음과 같은 오류가 발생할 수 있습니다:
...
이 오류는 일반적으로 CI 환경의 pnpm 버전과 로컬 개발 환경의 pnpm 버전이 다를 때 발생합니다. 로그에서 다음과 같은 경고를 확인할 수 있습니다:
...
GitHub Actions에서 pnpm을 사용하는 경우, 다음과 같이 설정을 업데이트하세요:
